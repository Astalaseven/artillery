...
if answer.lower() == "y" or answer.lower() == "yes":
    if operating_system == "posix":
        ...
        proc = subprocess.Popen("ps -au | grep /var/artillery/artillery.py",
                                stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
        stdout = proc.communicate()
        for line in stdout:
            match = re.search("python2 /var/artillery/artillery.py", line)
            if match:
                print "[*] Killing running version of Artillery.."
                line = line.split(" ")
                pid = line[6]
                subprocess.Popen("kill %s" % (
                    pid), stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True).wait()
                print "[*] Killed the Artillery process: " + pid
            match = re.search("python2 artillery.py", line)
            if match:
                print "[*] Killing running version of Artillery.."
                line = line.split(" ")
                pid = line[6]
        ...
        if os.path.isdir("/etc/init.d"):
            ...
            if os.path.isfile("/etc/init.d/rc.local"):
                ...
                data = fileopen.read()
                data = data.replace(
                    "sudo python2 /var/artillery/artillery.py &", "")
    ...
    if operating_system == "posix":
        ...
        if choice == "y" or choice == "yes":
            ...
            subprocess.Popen(
                "git clone https://github.com/trustedsec/artillery /var/artillery/", shell=True).wait()
            if os.path.isdir("/var/artillery/"):
                os.makedirs("/var/artillery/logs")
    ...
    if choice == "yes" or choice == "y":
        if operating_system == "posix":
            subprocess.Popen(
                "python2 /var/artillery/artillery.py &", shell=True).wait()
