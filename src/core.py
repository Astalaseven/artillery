    ...
    if ipcheck:
        ...
        if operating_system == "posix":
            ...
            if ip not in data:
                ...
                subprocess.Popen(
                    "iptables -I ARTILLERY 1 -s %s -j DROP" %
                    ip, shell=True).wait()
                filewrite.write(ip + "\n")
                filewrite.close()
        ...
        if operating_system == "windows":
            subprocess.Popen(
                "route ADD %s MASK 255.255.255.255 10.255.255.255" %
...
def prep_email(alert):
    ...
    if operating_system == "posix":
        ...
        filewrite = file(
            "/var/artillery/src/program_junk/email_alerts.log",
            "w")
    if operating_system == "windows":
        program_files = os.environ["ProgramFiles"]
        filewrite = file(
            program_files +
            "\\Artillery\\src\\program_junk\\email_alerts.log",
            "w")
...
def create_iptables():
    ...
    operating_system = check_os()
    if operating_system == "posix":
        # subprocess.Popen("iptables -F INPUT", stdout=subprocess.PIPE,
        # stderr=subprocess.PIPE, shell=True)
        subprocess.Popen(
            "iptables -N ARTILLERY",
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            shell=True)
        subprocess.Popen(
            "iptables -F ARTILLERY",
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            shell=True)
        subprocess.Popen(
            "iptables -I INPUT -j ARTILLERY",
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            shell=True)

    # sync our iptables blocks with the existing ban file so we don't forget
    # attackers
...
def threat_server():
    ...
    if os.path.isdir(public_http):
        while True:
            subprocess.Popen(
                "cp /var/artillery/banlist.txt %s" %
